defmodule Barbora.Provider.Slack do
  @hook_url Application.fetch_env!(:barbora, Barbora.Provider.Slack)[:hook_url]
  @predefined_blocks [
      %{
        type: "section",
        text: %{
  ...
  defp format_delivery_times(available_timeslots) do
    available_timeslots
    |>  Enum.reduce([], fn
      slot, acc -> [%{type: "section", text: %{type: "mrkdwn", text: "`#{slot["deliveryTime"]}`"}} | acc]
    end)
  end
end
